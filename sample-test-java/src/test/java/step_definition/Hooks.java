...
public class Hooks {
    ...
    public void testSetUp(Scenario scenario) throws FileNotFoundException {
        ...
        String tagname = tags.stream().filter(x -> x.startsWith("@testdata")).findAny().get();
        InputStream io = new FileInputStream("src/test/java/resources/test_data/" + tagname.split("=")[1]);
